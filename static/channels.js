...
$('#next-page').on('click', (evt) => {
    ...
    $.ajax({
      ...
      success: function(response) {
          ...
          $.each(response, function (i, item) {
            for (channel of item) {
              trHTML += '<tr><td>' + channel.title + '</td><td>' + channel.description + '</td><td>' + channel.video_count + '</td><td>' + channel.view_count + '</td><td>' + channel.subscriber_count + '</td><td>' + channel.published_date + '</td><td>' + "<a target='_blank' href=" + channel.url + ">Link</a>" + '</td></tr>';
            }
          });
          $('#results-table').append(trHTML);
      }
